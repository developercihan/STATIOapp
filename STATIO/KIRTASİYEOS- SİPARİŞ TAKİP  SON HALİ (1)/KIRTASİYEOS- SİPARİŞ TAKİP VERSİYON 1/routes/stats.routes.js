...
const companiesPath = path.join(dataAccess.dataDir, 'companies.json');
...
router.get('/dashboard', requireLogin, requireRole('admin'), async (req, res) => {
    try {
        ...
        const companies = await dataAccess.readJson(companiesPath);
...
router.get('/product/:code', requireLogin, requireRole('admin'), async (req, res) => {
    try {
        ...
        const productOrders = orders.filter(o => 
            ...
            date: o.createdAt,
            qty: (o.items.find(i => i.code === code).qty || o.items.find(i => i.code === code).miktar || 0),
            status: o.status
        }));
