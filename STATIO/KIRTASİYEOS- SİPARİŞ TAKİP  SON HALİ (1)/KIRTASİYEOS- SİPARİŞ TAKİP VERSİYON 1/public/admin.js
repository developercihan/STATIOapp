...
async function renderProductsTab() {
    try {
        ...
        products.forEach(p => {
            ...
            html += `<tr>
                <td onclick="${p.image ? `openLightbox('${p.image}')` : ''}" style="cursor:pointer;">
                    ${p.image ? `<img src="${p.image}" style="width:40px; height:40px; object-fit:cover; border-radius:4px; border:1px solid var(--neon-cyan);">` : `<div style="width:40px; height:40px; background:rgba(255,255,255,0.05); display:flex; align-items:center; justify-content:center; border-radius:4px; font-size:0.6em; color:var(--text-secondary);">FOTO YOK</div>`}
                </td>
                ...
                <td>${taxStr}</td>
                <td style="font-weight:bold; color: ${isLowStock ? 'var(--neon-red)' : 'var(--neon-green)'}">${p.stock || 0}</td>
                <td>
                    <button class="btn" style="padding:5px 10px; font-size:0.8em; border-color:var(--neon-cyan); color:var(--neon-cyan);" onclick="openStockModal('${p.kod}', '${p.ad.replace(/'/g, "\\'")}', ${p.stock || 0})">Stok</button>
                    <button class="btn" style="padding:5px 10px; font-size:0.8em; border-color:var(--neon-purple); color:var(--neon-purple);" onclick="openProductModal('${p.kod}', '${p.ad.replace(/'/g, "\\'")}', '${p.priceExclTax || 0}', '${p.taxRate || 20}', ${p.stock || 0}, ${p.minStock || 10}, '${p.image || ''}')">Düzenle</button>
...
window.toggleWarehouseSelect = function(role) {
    ...
    if(wrapper) wrapper.style.display = role === 'warehouse' ? 'block' : 'none';
}
    ...
    try {
        ...
        comps.forEach(c => {
            ...
            html += `<tr>
                ...
                <td>${c.ad || '-'}</td>
                <td>
                    <button class="btn" style="padding:5px 10px; font-size:0.8em; border-color:var(--neon-purple); color:var(--neon-purple);" onclick="openCompModal('${c.cariKod}', '${adSafe}', '${c.phone || ''}', '${c.email || ''}', '${c.discountRate || 0}')">Düzenle</button>
...
window.openCompModal = function(kod='', ad='', phone='', email='', discountRate=0) {
    window.resetModalBtn();
    const isEdit = !!kod;
    document.getElementById('modal-title').textContent = isEdit ? 'Kurum Düzenle' : 'Yeni Kurum';
    document.getElementById('modal-body').innerHTML = `
        <div class="form-group">
        ...
        <div class="form-group full-width">
            <label>Sabit İskonto (%) <span style="color:var(--neon-pink); font-size:0.8em;">(Siparişlerde otomatik uygulanır)</span></label>
            <input type="number" id="m-discountRate" value="${discountRate}" min="0" max="100" step="0.1">
        </div>
    `;
...
async function saveComp(isEdit) {
    ...
    try {
        ...
        closeModal(); renderCompaniesTab(); showToast('Kaydedildi');
    } catch(e) { showToast(e.message, 'error'); }
}
...
// --- USER MANAGEMENT (Mevcut logic eklendi/temizlendi) ---
...
// --- XML LOAD (Terminoloji temizlendi) ---
...
async function renderOrdersTab() {
    try {
        ...
        const compOptions = companies.map(c => `<option value="${c.kod}">${c.ad}</option>`).join('');
        ...
        let html = `
            <div class="action-bar">
                ...
                <div style="display:flex; gap:10px;">
                    ...
                    <button class="btn" style="border-color:var(--neon-cyan); color:var(--neon-cyan);" onclick="window.location.href='/api/orders/export.csv'">📊 Excel İndir</button>
                </div>
            </div>
...
window.updateBulkBtnVisibility = function() {
    ...
    document.getElementById('bulk-delete-btn').style.display = selected > 0 ? 'block' : 'none';
}
...
window.filterOrders = function() {
    ...
    document.querySelectorAll('#orders-tbody tr').forEach(row => {
        ...
        const rowCompany = row.cells[1].textContent;
